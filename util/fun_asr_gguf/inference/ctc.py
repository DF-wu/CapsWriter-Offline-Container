...
def _prepare_onnx_cuda_runtime(use_cuda: bool) -> None:
    if not use_cuda:
        return
...
class CTCDecoder:
    ...
    def __init__(
        ...
        try:
            ...
            if self.use_cuda or self.dml_enable:
                self.use_cuda = False
                self.dml_enable = False
    ...
    def _initialize_session(self):
        ...
        _prepare_onnx_cuda_runtime(self.use_cuda)

        available_providers = onnxruntime.get_available_providers()
        providers = ["CPUExecutionProvider"]
        if self.use_cuda and "CUDAExecutionProvider" in available_providers:
            providers.insert(0, "CUDAExecutionProvider")
        elif self.dml_enable and "DmlExecutionProvider" in available_providers:
            providers.insert(0, "DmlExecutionProvider")

        logger.info(
            f"[CTC] 加载模型: {os.path.basename(self.model_path)} (Providers: {providers})"
        )

        try:
            self.sess = onnxruntime.InferenceSession(
                self.model_path, sess_options=session_opts, providers=providers
            )
        except Exception:
            self.sess = onnxruntime.InferenceSession(
                self.model_path,
    ...
    def decode(
        self, enc_output: np.ndarray, enable_ctc: bool, max_hotwords: int
    ) -> Tuple[List[Token], List[str], Dict[str, float]]:
        """执行推理、解码和热词过滤"""
...
def decode_ctc(logits, id2token):
    ...
    timings = {"cast": t_cast, "argmax": t_argmax, "loop": t_loop}
    ...
    for i in range(1, n):
        for j in range(1, m):
            ...
            s_diag = score[i - 1][j - 1] + (
                match_score if char_ctc.lower() == char_llm.lower() else mismatch_score
            )
