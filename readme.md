...
| 🔄 **易跟進上游** | 主要加值都在新增目錄；目前只有 23 個 upstream-tracked 檔案有刻意差異，並已在同步 SOP 中列明。詳見 [docs/architecture.md](docs/architecture.md) |
...
打開 [`docker-compose.yml`](docker-compose.yml) 的 HTTP API port mapping（取消 `ports:` 底下 HTTP API mapping 的註解），然後：
...
| Client | 上游有完整 Windows client + GUI | 保留上游桌面 client；僅加 bounded local subprocess/request cleanup guards，避免 GUI 錄音、檔案轉錄與 standalone hotword helper 卡住 |
