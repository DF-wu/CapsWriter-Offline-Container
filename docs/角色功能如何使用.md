...
| 文件名 | 触发前缀 | 输出模式 | 用途 |
|--------|---------|---------|------|
| [`default.py`](../LLM/default.py) | （默认） | typing | 默认角色，用于润色，默认关闭，需填写自己的 API Key 后再启用 |
| [`翻译.py`](../LLM/翻译.py) | 翻译 | toast | 翻译选中文字，默认使用本地 LM Studio |
| [`小助理.py`](../LLM/小助理.py) | 小助理 | toast | 问答助手，默认使用本地 LM Studio |
| [`大助理.py`](../LLM/大助理.py) | 大助理 | toast | 联网模型助手，默认关闭且不内置 API Key |

联网服务模板默认不内置 API Key；启用 `openai`、`deepseek`、`moonshot`、`zhipu`、`claude`、`gemini` 等 provider 前，需要填入自己的 `api_key`，再把 `enabled` 改为 `True`。
...
### enabled
- `True`：启用此角色；有触发前缀的角色可被前缀匹配，默认角色可处理未匹配文本
- `False`：跳过此角色；如果默认角色也关闭，未匹配文本会直接输出原始识别结果
...
遍历所有角色
    ↓
跳过未启用角色，检查 text 是否以 role.names 之一开头
    ├─ 匹配成功 → 去掉前缀和分隔符 → 交给该角色处理
    └─ 全部不匹配 → 检查 default.enabled
        ├─ True → 交给默认角色处理
        └─ False → 不调用 LLM，直接输出
```
...
如果你希望所有语音都经过 LLM 润色，将 `default.py` 中的 `enabled = False` 改为 `enabled = True`，并填入自己的 `api_key`。
