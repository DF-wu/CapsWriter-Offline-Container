...
擷取亦會關閉即時時鐘、移除外部 CSS 字型網址；strict suite 會重新 render，
並要求結果與 committed SVG 完全一致。
...
| Python | Lock 支援 3.10 到 3.12；CI 以 Ubuntu 24.04／Windows 2022 × Python 3.10／3.12 四組執行完整 strict suite |
...
command 都會失敗。CI 會以 Ubuntu 24.04／Windows 2022 × Python 3.10／3.12
四組分別執行此 consumer。
...
從 Python 3.10 重新產生 universal lock，最後在 Ubuntu 與 Windows 跑兩個
interpreter gate（共四組 CI）。不可手動修改 lock 內的 version 或 hash。
