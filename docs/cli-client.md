...
Transcript files are written through a same-directory temporary file and atomic replace. In `--output-dir` batch mode, each successful transcript is written immediately instead of waiting for the entire batch; if a later file fails, earlier outputs remain durable. If the final replace fails, the previous transcript stays intact and the temporary file is removed.
