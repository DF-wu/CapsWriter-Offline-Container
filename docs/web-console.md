...
| 單元測試 | `npm run test` | API parsing、OpenAI-style / legacy / bounded non-JSON error parsing、readiness diagnostics 與 App render 測試通過 |
...
- HTTP error 會優先顯示 OpenAI-style `error.message`，並相容舊版 `detail` payload；非 JSON 錯誤 body 會壓成單行且限制長度，避免 proxy HTML 直接塞滿 UI。
