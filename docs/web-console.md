...
| 單元測試 | `npm run test` | API parsing、OpenAI-style / legacy / bounded non-JSON error parsing、invalid JSON diagnostics、bounded health/readiness/model diagnostics、recording cleanup、download object URL cleanup、TTS voice handler cleanup、clipboard copy denial、blocked localStorage handling、malformed settings/history recovery 與 App render 測試通過 |
...
- HTTP error 會優先顯示 OpenAI-style `error.message`，並相容舊版 `detail` payload；非 JSON 錯誤 body 會壓成單行且限制長度，避免 proxy HTML 直接塞滿 UI。Health/readiness/model list 診斷有 10 秒前端 timeout；若 `/ready` 或 JSON 格式轉錄回應不是合法 JSON，錯誤訊息會包含 HTTP status 與 endpoint，方便定位 proxy 或舊版 server 問題。
- TTS 目前是 browser-local Web Speech API；不把音訊傳到雲端。
- localStorage 只保存非敏感使用者設定與最近 20 筆轉錄歷史；手動輸入的 API key 只留在目前頁面記憶體中。若瀏覽器封鎖 storage 或 quota 用完，保存會 best-effort 失敗但不阻斷目前操作。
