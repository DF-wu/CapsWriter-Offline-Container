...
- 轉錄進行中會鎖定音訊替換入口；使用者需按「取消」或等待完成後再換檔，避免舊請求完成後覆蓋新音訊狀態。
