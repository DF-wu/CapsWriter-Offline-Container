...
Maintainers publish the static image through [`.github/workflows/publish-web-image.yml`](../.github/workflows/publish-web-image.yml). The workflow first runs `python scripts/verify_all.py --docker-build-web`, including the production Nginx image smoke, then builds from [`client/web/Dockerfile`](../client/web/Dockerfile), pushes `ghcr.io/<owner>/capswriter-offline-web:{latest,sha-*}`, and publishes provenance/SBOM attestations. The Node and Nginx base images are pinned with `tag@sha256` references.
...
- localStorage 保存使用者設定與最近 20 筆明文轉錄／raw 歷史；轉錄內容可能是敏感資料，共用電腦使用後應清除 history／site data，且不可把 browser storage 當 encrypted records system。Settings controls 會套用同一組字串長度上限，手動輸入的 API key 只留在目前頁面記憶體中。讀回時會檢查型別、格式與字串長度，過大或 malformed record 會被忽略。若瀏覽器封鎖 storage 或 quota 用完，保存會 best-effort 失敗但不阻斷目前操作。
- 「清除全部歷史」會先顯示不可復原的確認提示；取消時不會修改任何 record。
- 所有 diagnostic 與 multipart transcription fetch 都使用 `redirect: "error"`；browser 不會把 API key 或私密音訊重送到 redirect target。
