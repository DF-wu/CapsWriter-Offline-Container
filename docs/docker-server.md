...
本文件只覆蓋 **Server 端**。Client 仍維持上游原本 Windows 用法，本 fork 不變更。
...
支援的模型（對應 upstream v2.5 release）：

| 模型 | Release zip | sha256 對齊 |
|---|---|---|
| qwen_asr | `Qwen3-ASR-1.7B-q5_k.zip` | ✅ |
| fun_asr_nano | `Fun-ASR-Nano-GGUF.zip` | ✅ |
| sensevoice | `sherpa-onnx-sense-voice-zh-en-ja-ko-yue-2024-07-17.zip` | 路徑已對齊上游 attribute（內容可能需驗證） |
| paraformer | `speech_paraformer-large-vad-punc_asr_nat-zh-cn-16k-common-vocab8404-onnx.zip` + 標點 | ✅ |
