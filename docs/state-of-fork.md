...
| `python scripts/verify_all.py` | 通過：upstream divergence guard、CLI 43 tests + packaged stdin smoke、server compile、HTTP 70 tests（含 server key-file auth、translations endpoint auth consistency、bounded language hints、late canceled HTTP result absorption 與 ws_send drift guard）、Docker server 22 tests（含 bounded atomic model archive downloads 與 clean download-failure reporting）、Verifier/diagnostic 39 tests、Web 69 tests/build（含 API root validation before requests、abortable diagnostics、keyboard-accessible upload、drag/drop highlight stability、transcription-time audio replacement lock、stale result suppression after cancel/unmount、late diagnostic/clipboard suppression、recording startup cleanup success/failure、TTS voice lifecycle/callback cleanup、download filename sanitization、bounded settings controls/storage、malformed history/runtime-config filtering）、cleanup + residue check |
