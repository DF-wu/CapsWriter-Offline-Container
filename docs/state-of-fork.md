...
| | |
|---|---|
| 修改的 upstream-tracked 檔案 | **7**：`.gitignore`、`readme.md`、`requirements-server.txt`、`LLM/default.py`、`assets/BUILD_GUIDE.md`、`zip_release.py`、`core/tools/window_detector.py` |
...
| `python -m unittest discover -s scripts/tests -v` | 通過：Verifier/diagnostic 66 tests，含 upstream divergence guard bounded git subprocess handling、root verifier per-command timeout/redacted timeout reporting、bounded Web clean/verify subprocess handling、bounded browser-smoke `agent-browser` subprocess/child cleanup handling、release ZIP packaging timeout/temp-file cleanup guards、desktop window detector bounded `osascript`/`wmctrl` subprocess guards、live HTTP API key log redaction/key-file pass-through/empty-file rejection、diagnostic host/port validation、diagnostic streamed multipart/bounded response body reads/server-aligned default timeout/configurable timeout/real HTTP body delivery/POST 401 handling、Docker Compose HTTP/model tuning env guard、HTTP API dependency guard、role template secret/default guard、`/health` 401 API-key guidance、cleanup traversal/residue gate 與 HTTP decode timeout source guard |
