...
| Gate | 結果 |
|---|---|
| `python -m unittest discover -s client/cli/tests -v` | 通過：CLI 15 tests，含 `/ready` ok/degraded diagnostic command、OpenAI-style error parsing、non-JSON/invalid JSON diagnostics 與 multipart filename escaping |
...
| `python scripts/verify_all.py --web-browser-smoke --docker-build-web --http-base-url http://127.0.0.1:6017` | 通過：CLI 15 tests、server compile、HTTP 50 tests、Docker server 12 tests、Verifier/diagnostic 12 tests、Web 27 tests/build、browser health/readiness/upload/transcribe smoke、Web Docker smoke、live `/health` |
