...
- 支援錄音、鍵盤可操作的檔案選擇、拖放上傳、播放、STT、五種輸出格式、HTTP readiness diagnostics、歷史紀錄、下載、browser Web Speech TTS；轉錄中會鎖定音訊替換，取消/換檔/卸載後會忽略 stale result，卸載後也會忽略晚到的 diagnostic、voice loading、speech callback 與 media stream。
...
| `python scripts/verify_all.py` | 通過：upstream divergence guard、CLI 39 tests + packaged stdin smoke、server compile、HTTP 70 tests（含 server key-file auth、translations endpoint auth consistency、bounded language hints、late canceled HTTP result absorption 與 ws_send drift guard）、Docker server 17 tests、Verifier/diagnostic 39 tests、Web 64 tests/build（含 API root validation before requests、keyboard-accessible upload、drag/drop highlight stability、transcription-time audio replacement lock、stale result suppression after cancel/unmount、late diagnostic suppression、recording startup cleanup、TTS voice lifecycle/callback cleanup、download filename sanitization、bounded settings controls/storage、malformed history/runtime-config filtering）、cleanup + residue check |
