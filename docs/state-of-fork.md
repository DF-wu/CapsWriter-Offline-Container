...
| `python -m unittest discover -s scripts/tests -v` | 通過：Verifier/diagnostic 6 tests，含 live HTTP API key log redaction 與 diagnostic multipart filename escaping |
| `python scripts/verify_all.py --web-browser-smoke --docker-build-web --http-base-url http://127.0.0.1:6017` | 通過：CLI 15 tests、server compile、HTTP 45 tests、Docker server 12 tests、Verifier/diagnostic 6 tests、Web 22 tests/build、browser health/readiness/upload/transcribe smoke、Web Docker smoke、live `/health` |
