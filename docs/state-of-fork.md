...
| Gate | 結果 |
|---|---|
| `python -m unittest discover -s client/cli/tests -v` | 通過：CLI 51 tests，含 `/ready` ok/degraded diagnostic command、key-file auth/empty-file rejection、base URL validation before requests、server-aligned default timeout/positive timeout validation、valid JSON output files、portable sanitized `--output-dir` target generation/duplicate rejection、OpenAI-style error parsing、non-JSON/invalid JSON diagnostics、streamed multipart upload、multipart filename escaping、`speak --stdin` pipeline input、bounded local TTS timeout handling 與 standalone CLI verifier timeout handling |
