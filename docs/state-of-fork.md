...
| `python -m unittest discover -s docker/server/tests -v` | 通過：Docker server 8 tests，含 HTTP `/ready` healthcheck 與 model downloader env diagnostics |
| `python scripts/verify_all.py --web-browser-smoke --docker-build-web --http-base-url http://127.0.0.1:6017` | 通過：CLI 10 tests、server compile、HTTP 42 tests、Docker server 8 tests、Web 18 tests/build、browser health/readiness/upload/transcribe smoke、Web Docker smoke、live `/health` |
