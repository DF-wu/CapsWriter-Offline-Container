...
| Gate | 結果 |
|---|---|
| `python -m unittest discover -s client/cli/tests -v` | 通過：CLI 20 tests，含 `/ready` ok/degraded diagnostic command、key-file auth、OpenAI-style error parsing、non-JSON/invalid JSON diagnostics、streamed multipart upload 與 multipart filename escaping |
...
| `python scripts/verify_all.py --web-browser-smoke --docker-build-web --http-base-url http://127.0.0.1:6017` | 通過：CLI 20 tests、server compile、HTTP 51 tests、Docker server 12 tests、Verifier/diagnostic 23 tests、Web 34 tests/build、browser health/readiness/upload/transcribe smoke、Web Docker smoke、live `/health` |
