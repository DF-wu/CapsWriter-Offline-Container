...
| `python -m unittest discover -s scripts/tests -v` | 通過：Verifier/diagnostic 7 tests，含 live HTTP API key log redaction、diagnostic multipart filename escaping 與 `/health` 401 API-key guidance |
| `python scripts/verify_all.py --web-browser-smoke --docker-build-web --http-base-url http://127.0.0.1:6017` | 通過：CLI 15 tests、server compile、HTTP 48 tests、Docker server 12 tests、Verifier/diagnostic 7 tests、Web 24 tests/build、browser health/readiness/upload/transcribe smoke、Web Docker smoke、live `/health` |
