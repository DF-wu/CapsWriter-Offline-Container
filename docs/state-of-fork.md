...
- [`.github/workflows/publish-server-image.yml`](../.github/workflows/publish-server-image.yml) 先跑 `python scripts/verify_all.py --skip-web`，通過後以 job-scoped `packages: write` 發 server image，並產出 provenance/SBOM attestations。
- [`.github/workflows/publish-web-image.yml`](../.github/workflows/publish-web-image.yml) 先跑 `python scripts/verify_all.py --docker-build-web`，含 Web Docker smoke，通過後以 job-scoped `packages: write` 發 Web Console image，並產出 provenance/SBOM attestations。
