...
- [`docker/server/download_models.py`](../docker/server/download_models.py) 依 `CAPSWRITER_MODEL_TYPE` 自動下載模型，archive 下載有 idle timeout，且先寫 `.part` 再 atomic replace。
...
- `transcribe --output` / `--output-dir` 以 same-directory temp file + atomic replace 寫入 transcript，replace 失敗時保留舊檔並清掉暫存檔。
...
- 支援錄音、鍵盤可操作的檔案選擇、拖放上傳、播放、STT、五種輸出格式、可 abort 的 HTTP readiness diagnostics、歷史紀錄、下載、browser Web Speech TTS；轉錄中會鎖定音訊替換，取消/換檔/卸載後會忽略 stale result，卸載後也會忽略晚到的 diagnostic、clipboard、voice loading、speech callback 與 media stream。
...
| Gate | 結果 |
|---|---|
| `python -m unittest discover -s client/cli/tests -v` | 通過：CLI 53 tests，含 `/ready` ok/degraded diagnostic command、key-file auth/empty-file rejection、base URL validation before requests、server-aligned default timeout/positive timeout validation、valid JSON output files、atomic transcript output writes/temp cleanup、portable sanitized `--output-dir` target generation/duplicate rejection、OpenAI-style error parsing、non-JSON/invalid JSON diagnostics、streamed multipart upload、multipart filename escaping、`speak --stdin` pipeline input、bounded local TTS timeout handling 與 standalone CLI verifier timeout handling |
| `python -m unittest discover -s docker/server/tests -v` | 通過：Docker server 22 tests，含 HTTP `/ready` healthcheck、healthcheck env parsing、model downloader bounded atomic download/timeout diagnostics、clean download-failure reporting、llama.cpp runtime library readiness 與 entrypoint Qwen CPU preset guard |
| `python -m unittest discover -s scripts/tests -v` | 通過：Verifier/diagnostic 49 tests，含 upstream divergence guard bounded git subprocess handling、root verifier per-command timeout/redacted timeout reporting、bounded Web clean subprocess handling、live HTTP API key log redaction/key-file pass-through/empty-file rejection、diagnostic host/port validation、diagnostic streamed multipart/server-aligned default timeout/configurable timeout/real HTTP body delivery/POST 401 handling、Docker Compose HTTP/model tuning env guard、HTTP API dependency guard、role template secret/default guard、`/health` 401 API-key guidance、cleanup traversal/residue gate 與 HTTP decode timeout source guard |
...
| `python scripts/verify_all.py` | 通過：upstream divergence guard（含 bounded git subprocess handling）、CLI 53 tests + packaged stdin smoke（含 atomic transcript output writes/temp cleanup、standalone verifier timeout handling 與 portable `--output-dir` filename sanitization/collision guard）、server compile、HTTP 72 tests（含 server key-file auth、translations endpoint auth consistency、bounded ffmpeg timeout validation/kill cleanup、bounded language hints、late canceled HTTP result absorption 與 ws_send drift guard）、Docker server 22 tests（含 bounded atomic model archive downloads 與 clean download-failure reporting）、Verifier/diagnostic 49 tests（含 root verifier per-command timeout/redacted timeout reporting 與 bounded Web clean subprocess handling）、Web 69 tests/build（含 API root validation before requests、abortable diagnostics、keyboard-accessible upload、drag/drop highlight stability、transcription-time audio replacement lock、stale result suppression after cancel/unmount、late diagnostic/clipboard suppression、recording startup cleanup success/failure、TTS voice lifecycle/callback cleanup、download filename sanitization、bounded settings controls/storage、malformed history/runtime-config filtering）、cleanup + residue check |
