...
| `python -m unittest discover -s scripts/tests -v` | 通過：Verifier/diagnostic 16 tests，含 live HTTP API key log redaction、diagnostic streamed multipart/configurable timeout/real HTTP body delivery、HTTP API dependency guard、`/health` 401 API-key guidance、cleanup traversal pruning 與 HTTP decode timeout source guard |
| `python scripts/verify_all.py --web-browser-smoke --docker-build-web --http-base-url http://127.0.0.1:6017` | 通過：CLI 17 tests、server compile、HTTP 51 tests、Docker server 12 tests、Verifier/diagnostic 16 tests、Web 34 tests/build、browser health/readiness/upload/transcribe smoke、Web Docker smoke、live `/health` |
