...
| Web | `npm ci --no-audit --no-fund` then `npm run verify` in `client/web` | React/Vite tests, bounded Web API error/invalid JSON/diagnostic request timeout handling, API root validation before requests, partial readiness diagnostics when model listing fails, keyboard-accessible audio upload, stable drag/drop highlighting, transcription-time audio replacement locking, stale transcription result suppression after cancel, recording resource cleanup, deferred export URL cleanup, sanitized download filenames including Windows reserved device names, TTS voice handler cleanup, clipboard failure handling, best-effort localStorage persistence, bounded settings controls and malformed settings/history/runtime-config recovery, runtime `/config.js` escaping, TypeScript, production build, web clean script |
