...
| Server | `python -m compileall fork_server docker/server check_http_api.py start_server_docker.py` + HTTP and Docker server unit tests | HTTP sidecar, strict Bearer header parsing, server/client key-file auth support, late canceled/timeout HTTP result absorption, HTTP-aware `ws_send` upstream drift guard, Docker entrypoint/healthcheck syntax, readiness-gated container healthcheck, privacy-preserving transcript logging, SRT/VTT timestamp formatting, bounded ffmpeg decode diagnostics, model downloader diagnostics including runtime-linked llama libraries, dependency-light request limit, runtime config tests, and fail-fast server/model env validation |
