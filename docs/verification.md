...
|---|---|---|
| CLI | `python client/cli/scripts/verify.py` | CLI syntax, health/readiness/models calls, key-file auth, positive timeout validation, streamed multipart upload including filename escaping and early error responses, mock HTTP transcription, HTTP and invalid JSON diagnostics, output files, Linux/Windows TTS command selection, zipapp packaging |
