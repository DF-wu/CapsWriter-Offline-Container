...
| `CAPSWRITER_SERVER_MAX_WEBSOCKET_CONNECTIONS` | `8` | 同時 admitted 的 WebSocket client 上限；超額以 `1013` 拒絕 |
| `CAPSWRITER_SERVER_MAX_WEBSOCKET_TASK_SECONDS` | `3600` | 單一 WebSocket task 的累計音訊秒數上限；超出時只終止該 composite stream |
...
| `CAPSWRITER_SERVER_MAX_WEBSOCKET_CONNECTIONS` | `1..1024` |
| `CAPSWRITER_SERVER_MAX_WEBSOCKET_TASK_SECONDS` | `1..86400` 秒 |
...
environment:
  ...
  CAPSWRITER_HTTP_API_KEY: "${CAPSWRITER_HTTP_API_KEY}"  # 由 host 環境注入
  # 或用 secret file:
...
| `model` | 是 | — | 必須為 `whisper-1` 的 OpenAI 相容 ID；**實際模型由 `CAPSWRITER_MODEL_TYPE` 決定** |
...
`verbose_json.language` 不是 detected-language metadata。有傳入 hint 時回正規化後的
hint；未傳時固定為本機 sentinel `"auto"`，因為目前共用 `Result` 沒有可靠且跨
backend 一致的 detected-language 欄位。
...
| `503` | recognizer 不可用或已進入 fail-stop；等待 supervisor 重啟 |
| `504` | 任務超過 `TASK_TIMEOUT` |

### 2.2 `POST /v1/audio/translations`

明確回傳 `501 Not Implemented`。CapsWriter 本地模型不做語種翻譯。若伺服器啟用 Bearer auth，此端點仍會先驗證 `Authorization` header，未授權請求回 `401` 而不是 `501`。
...
部署用 readiness/diagnostic endpoint。它不需要 API key，也不暴露 token；用來確認 HTTP sidecar 已綁定 `task_router`、recognizer child 仍存活，且系統可找到 `ffmpeg`。
...
OpenAI SDK 某些初始化路徑會打這個。回應一筆 wire compatibility model 紀錄；實際 engine 仍由 `CAPSWRITER_MODEL_TYPE` 決定：
...
# verbose JSON + 認證
export CAPSWRITER_HTTP_API_KEY="$(< /secure/path/capswriter-http.key)"
...
HTTP request 成功、timeout、server error、upload 後斷線或客戶端取消時，都會清理 pending future 與合成 socket id；result 與 disconnect 同時完成時 result 優先，disconnect watcher 一定會回收。尚未開始的 Task 也會在 worker ingress 與推論前各做一次 socket/deadline recheck。Recognizer startup model load 受 `CAPSWRITER_SERVER_MODEL_LOAD_TIMEOUT`（預設 600 秒）限制，超時會先 terminate/kill/reap child 再讓啟動失敗。Parent 另監看同步 inference lease：HTTP deadline 加兩秒 grace 仍未返回，或任何 call 超過 `CAPSWRITER_SERVER_WORKER_STALL_TIMEOUT`（預設 900 秒），會 bounded terminate/kill child 並讓整個 server fail-stop。由 supervisor 重啟整個服務；不 hot-restart shared worker，以免破壞 worker 內 desktop/WebSocket session。

### 4.4 並發

- **識別嚴格串行**：recognizer 一次處理一個 Task，多個 HTTP 請求 = 自然 FIFO backpressure。
- **跨 process staging 有硬上限**：`queue_in` 最多 8 個 Task，worker 公平 buffer 另最多 8 個；每段最長 64 秒 float32／16 kHz／mono PCM，恰為 4,096,000 bytes。HTTP/WS producer 都等待容量，WS 以 non-blocking put cooperative retry，不會阻塞 asyncio loop；單一 admitted WS frame 最多 6 MiB且只預取一個 message，預設同時最多 8 個 admitted connection。單一 WS task 預設最多累計 3,600 秒音訊；超限會回 final `websocket_task_audio_limit_exceeded`，並只清除該 `(socket_id, task_id)` cache／worker session。
- **結果派發有硬上限**：`queue_out` 最多 8 筆；每個 WebSocket peer 只有一個 active send 與最多 8 個依 task 排序的 pending snapshot。同 task intermediate 可 coalesce，不同 task final 會保留；send 超過五秒或 overflow 只隔離該 peer，不會阻塞 HTTP result 或其他 client。
...
預設兩個 active HTTP request 的 upload 與 decoded PCM 額度為
`2 × (104,857,600 + 230,400,000) = 670,515,200` bytes。再計入 8 格 input queue、
8 格 worker buffer、一個 active worker segment，以及兩個 HTTP producer 各自可能
blocked 的一段，保守 logical HTTP audio-payload ceiling 恰為 `748,339,200` bytes
（713.671875 MiB）。這不含模型、allocator、ffmpeg working memory、序列化瞬間 copy，
也不含依連線個別有界的 WebSocket frame/cache。
