...
class UpstreamRefreshTest(unittest.TestCase):
    ...
    def test_failed_native_model_load_stops_before_vocab_access(self):
        vocab = Mock()
        namespace = load_definitions(
            "core/server/engines/llama/llama.py", {"LlamaModel"},
            Path=Path, ctypes=ctypes,
            llama_model_default_params=lambda: SimpleNamespace(),
            llama_model_load_from_file=Mock(return_value=None),
            llama_model_get_vocab=vocab,
        )
        with self.assertRaisesRegex(RuntimeError, 'missing.gguf'):
            namespace['LlamaModel']('missing.gguf', use_gpu=False)
