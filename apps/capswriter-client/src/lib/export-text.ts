...
export function transcriptMarkdownExport(transcript: string, rawResult: string): TextExport {
  ...
  return {
    filename: `capswriter-transcript-${safeTimestamp(timestamp)}.md`,
    mimeType: "text/markdown",
    title: "CapsWriter transcript",
    text: [
      "# CapsWriter Transcript",
      "",
      `Exported: ${timestamp}`,
      "",
      "## Transcript",
...
export function conversationMarkdownExport(messages: ChatMessage[]): TextExport {
  const timestamp = new Date().toISOString();
  return {
    filename: `capswriter-conversation-${safeTimestamp(timestamp)}.md`,
    mimeType: "text/markdown",
    title: "CapsWriter conversation",
    text: [
      "# CapsWriter Conversation",
      "",
      `Exported: ${timestamp}`,
      "",
